...
const HIT_BOX = {   // こうもりの当たり判定 (タイル基準。すなわち 1 ならタイル1枚分)
    width: 0.35,    // 横幅
    height: 0.35,   // 縦幅
}
...
export class Bat{
    ...
    //     2-1. ランダムな整数が  1 ~  60 の場合、今まで動いてた方向にそのまま動く
    //     2-2. ランダムな整数が 61 ~  70 の場合、上に動く
    //     2-3. ランダムな整数が 71 ~  80 の場合、下に動く
    //     2-4. ランダムな整数が 81 ~  90 の場合、左に動く
    //     2-5. ランダムな整数が 91 ~ 100 の場合、右に動く
    control(_player){
        ...
        const decide_direction_from_random_int = (random_num) => {
            if( 1 <= random_num && random_num <=  60) return this.direction;
            if(61 <= random_num && random_num <=  70) return DIRECTION.up;
            if(71 <= random_num && random_num <=  80) return DIRECTION.down;
            if(81 <= random_num && random_num <=  90) return DIRECTION.left;
            if(91 <= random_num && random_num <= 100) return DIRECTION.right;
        }
    ...
    draw(_canvas, context, tile_size_in_canvas){
        this.draw_myself(_canvas, context, tile_size_in_canvas, ANIMATION_ORDER);
    }
}
...
include(Bat, attack);
include(Bat, damaged);
include(Bat, is_damaged);
include(Bat, is_blown_away);

// 03_draw
include(Bat, draw_myself);

// その他
include(Bat, is_overlapping_with);
