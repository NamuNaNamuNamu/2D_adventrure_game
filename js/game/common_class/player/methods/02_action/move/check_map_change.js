...
export function check_map_change(img, enemies){
    if(this.x <= 0 - MINIMUM_STEP){
        this.x += (FIELD_SIZE_IN_SCREEN + MINIMUM_STEP);
        this.world_map_x--;
        this.execute_common_process_by_map_change(img, enemies);
    }
    if(this.x >= FIELD_SIZE_IN_SCREEN + MINIMUM_STEP){
        this.x -= (FIELD_SIZE_IN_SCREEN + MINIMUM_STEP);
        this.world_map_x++;
        this.execute_common_process_by_map_change(img, enemies);
    }
    if(this.y <= 0 - MINIMUM_STEP){
        this.y += (FIELD_SIZE_IN_SCREEN + MINIMUM_STEP);
    ...
    if(this.y >= FIELD_SIZE_IN_SCREEN + MINIMUM_STEP){
        this.y -= (FIELD_SIZE_IN_SCREEN + MINIMUM_STEP);
        this.world_map_y++;
        this.execute_common_process_by_map_change(img, enemies);
    }
